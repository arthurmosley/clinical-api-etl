...
def read_csv_to_df(csv_path: str) -> pd.DataFrame:
    ...
    if df["participant_id"].str.strip().eq("").any():
        raise ValueError("participant_id is required for all rows and cannot be blank")
    if df["site_id"].str.strip().eq("").any():
        raise ValueError("site_id is required for all rows and cannot be blank")        
    return df
...
def process_job(job_id: str, csv_path: str) -> None:
    try:
        ...
        stage_dataframe(job_id, filename, df)

        mark_status(job_id, "running", "dims")
        set_progress(job_id, 45, "upserting dimensions")
        upsert_dimensions_for_job(job_id)
        
        mark_status(job_id, "running" "qc")
        set_progress(job_id, 50, "quality checks")
        create_QC_view()
        upsert_rejected(job_id)

        mark_status(job_id, "running", "processed")
        set_progress(job_id, 65, "building processed scalar and bp")
        upsert_scalar_facts(job_id)
        upsert_bp_facts(job_id)
